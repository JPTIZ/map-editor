...
def export(
    data, output="output", template="template", namespace="resources", exts=("h", "cpp")
):
    def render(f, template, env, data):
        MAP_SIZES = {
            (32, 32): "gba::graphics::MapSize::TEXT_256X256",
        }
        tileset, _ = splitext(basename(data.tileset.filename).replace("-", "_"))
    ...
    env = Environment(
        trim_blocks=True, autoescape=False, loader=FileSystemLoader(abspath("."))
    )
